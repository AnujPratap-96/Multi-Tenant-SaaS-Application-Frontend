            ...
            <div key={key} className={`rounded-xl border ${color} ${bg}`}>
              <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-100 dark:border-gray-800">
                ...
                <span className="ml-auto text-xs font-semibold text-gray-400 bg-white dark:bg-gray-800 px-2 py-0.5 rounded-full">
                  {stats.taskStats?.[key] ?? 0}
                </span>
              </div>
              <div className="p-3 min-h-[200px] space-y-2">
                {(stats.taskStats?.[key] ?? 0) === 0 ? (
                  <p className="text-sm text-gray-400 text-center py-8">No tasks</p>
                ) : (
                  <div className="text-center py-8">
                    <p className="text-sm text-gray-400">
                      {stats.taskStats?.[key]} task{stats.taskStats?.[key] !== 1 ? "s" : ""} in this column
                    </p>
                    <p className="text-xs text-gray-400 mt-1">
                      Full task cards will render when the Tasks module is connected.
                    </p>
                  </div>
                )}
              </div>
